...
def public_device(device: Device, online: bool = True) -> dict[str, Any]:
    return {
        ...
        "context_length": device.context_length,
        "mode": device.mode,
        "model": device.model,
...
app = FastAPI(title="AI Palm Coordinator", version="0.8.0", lifespan=lifespan, docs_url=None, redoc_url=None)
...
async def api_status(session: AsyncSession = Depends(get_session)):
    ...
    return {"ok": redis_ok, "version": "0.8.0", "database": "ok", "realtime": "ok" if redis_ok else "unavailable"}
...
async def register_host(
    ...
    if device is None:
        ...
        device.context_length = payload.context_length
...
async def stream_job(
    ...
    if payload.thinking_delta:
        job.thinking = (job.thinking + payload.thinking_delta)[: settings.max_result_chars]
        if not payload.delta:
            job.note = "يفكر النموذج الآن"
